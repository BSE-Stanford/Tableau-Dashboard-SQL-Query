SELECT
    ...
    'Credit Card Services'                    AS payment_group
FROM
...
WHERE
   ...
   -- AND t.assignment_group IN ( 'FMS Credit Card Services' )
   AND ( t.assignment_group IN ( 'FMS Credit Card Services' )
                  OR EXISTS (
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND o_r.value IN ( 'FMS Credit Card Services' )
                    AND o_r.end_time IS NOT NULL
            ) )
    --AND t.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-4)
...
FROM
    (
        ...
        WHERE
            ...
            AND ( m.assignment_group IN ( 'FMS Credit Card Services' )
                  OR EXISTS (
                SELECT
                    1
                FROM
			...
			AND extract(year from m.sys_updated_on ) >= (extract( year from current_date )-4)
...
WHERE
    r.assignment_group IN ( 'FMS Credit Card Services' )
--and parent is not null
    AND r.number LIKE 'TASK%'
    --AND r.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from R.sys_updated_on ) >= (extract( year from current_date )-4)
/*
UNION ALL
SELECT
    ROW_NUMBER()
    ...
    t.requested_for_cost_center,
    t.requested_for_department,
    'Child SR Task Created By Team'   AS        type,
    'Credit Card Services'                    AS payment_group
FROM
    support_requests.support_task t
    JOIN support_requests.support_request     m ON m.number = t.parent
...
WHERE
    --r.field = 'assigned_to'
    r.definition = 'SU Support Req. Task State Change'
    AND t.parent = m.number
    AND m.assignment_group IN ( 'FMS Credit Card Services' )
    AND t.assignment_group NOT IN ( 'FMS Credit Card Services' )
...
SELECT
    ...
    t.requested_for_department,
    'SR Task Assigned To Team'       AS         type,
    'Payment Operations'                      AS payment_group
FROM
    support_requests.support_task t
    JOIN support_requests.support_request     m ON m.number = t.parent
    LEFT JOIN support_requests.metric_support_task r ON r.number = t.number
WHERE
    r.definition = 'SU Support Req. Task State Change'
    --AND t.assignment_group IN ( 'FMS Payment Operations' )
    --AND t.sys_updated_on  >= ( current_date - 2000 )
	AND ( t.assignment_group IN ( 'FMS Payment Operations' )
                  OR EXISTS (
                SELECT
                ...
                WHERE
                    o_r.number = t.number
                    AND o_r.definition = 'SU Support Req. Task Assignment Group'
                    AND o_r.value IN ( 'FMS Payment Operations' )
                    AND o_r.end_time IS NOT NULL
            ) )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-4)
...
FROM
    (
        ...
        WHERE
            r.definition = 'SU Support Request State Change'
            AND ( m.assignment_group IN ( 'FMS Payment Operations' )
                  OR EXISTS (
                SELECT
                    1
                FROM
...
FROM
    ...
    JOIN requests.request_item m ON r.parent = m.number
WHERE
    r.assignment_group IN ( 'FMS Payment Operations' )
--and parent is not null
    AND r.number LIKE 'TASK%'
    --AND r.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from R.sys_updated_on ) >= (extract( year from current_date )-4)
...
WHERE
    ...
    AND t.parent = m.number
    AND m.assignment_group IN ( 'FMS Payment Operations' )
    AND t.assignment_group NOT IN ( 'FMS Payment Operations' )
    --AND t.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-5)
*/
UNION ALL
SELECT
    ...
    t.requested_for_department,
    'SR Task Assigned To Team'            AS    type,
    'Accounts Payable'                        AS payment_group
FROM
    support_requests.support_task t
    JOIN support_requests.support_request     m ON m.number = t.parent
    LEFT JOIN support_requests.metric_support_task r ON r.number = t.number
WHERE
    r.definition = 'SU Support Req. Task State Change'
    --AND t.assignment_group IN ( 'FMS Accounts Payable' )
    --AND t.sys_updated_on  >= ( current_date - 2000 )
	AND ( t.assignment_group IN ( 'FMS Accounts Payable' )
                  OR EXISTS (
                SELECT
                ...
                WHERE
                    o_r.number = t.number
                    AND o_r.definition = 'SU Support Req. Task Assignment Group'
                    AND o_r.value IN ( 'FMS Accounts Payable' )
                    AND o_r.end_time IS NOT NULL
            ) )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-4)
...
FROM
    (
        ...
        WHERE
            r.definition = 'SU Support Request State Change'
            AND ( m.assignment_group IN ( 'FMS Accounts Payable' )
                  OR EXISTS (
                SELECT
                    1
                FROM
...
FROM
    ...
    JOIN requests.request_item m ON r.parent = m.number
WHERE
    r.assignment_group IN ( 'FMS Accounts Payable' )
--and parent is not null
    AND r.number LIKE 'TASK%'
    --AND r.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from R.sys_updated_on ) >= (extract( year from current_date )-4)
...
WHERE
    ...
    AND t.parent = m.number
    AND m.assignment_group IN ( 'FMS Accounts Payable' )
    AND t.assignment_group NOT IN ( 'FMS Accounts Payable' )
    --AND t.sys_updated_on  >= ( current_date - 2000 )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-5)*/