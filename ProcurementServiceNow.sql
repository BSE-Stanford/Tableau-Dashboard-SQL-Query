SELECT
	...
	'N/A' as survey_comments,
	'N/A' as survey_response,
    'SR Task Assigned To Team'          AS      type,
    'Business Expense'                        AS procurement,
    current_date                              AS report_last_update_date
FROM
...
WHERE
    ...
    AND t.assignment_group IN ( 'FMS Business Expense' )
...
FROM
    (
        ...
        WHERE
            ...
            AND ( m.assignment_group IN ( 'FMS Business Expense' )
                  OR EXISTS (
                SELECT
                    1
                FROM
                ...
                WHERE
                    ...
                    AND o_r.value IN ( 'FMS Business Expense' )
...
WHERE
    r.assignment_group IN ( 'FMS Business Expense' )
...
SELECT
    ...
    m.requested_by_cost_center,
    m.requested_by_department,
    --m.requested_by_sys_created_by             requested_by_user_name,
    m.requested_by_user_name,
    /*concat(
        t.requested_for_first_name, ' ', t.requested_for_last_name
    )*/
...
WHERE
    ...
    AND m.assignment_group IN ( 'FMS Business Expense' )
    AND t.assignment_group NOT IN ( 'FMS Business Expense' )
    --AND m.sr_created_on_date >= ( current_date - 3000 )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-5)
...
WHERE
    r.definition = 'SU Support Req. Task State Change'
    AND t.assignment_group IN ( 'FMS Travel' )
    --AND m.sr_created_on_date >= ( current_date - 3000 )
	AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-5)
...
FROM
    (
        ...
        WHERE
            r.definition = 'SU Support Request State Change'
            AND ( m.assignment_group IN ( 'FMS Travel' )
                  OR EXISTS (
                SELECT
                    1
                FROM
...
FROM
    ...
    JOIN requests.request_item m ON r.parent = m.number
WHERE
    r.assignment_group IN ( 'FMS Travel' )
--and parent is not null
    AND r.number LIKE 'TASK%'
    --AND r.opened_at >= ( current_date - 3000 )
	AND extract(year from r.sys_updated_on ) >= (extract( year from current_date )-5)
...
WHERE
    ...
    AND t.parent = m.number
    AND m.assignment_group IN ( 'FMS Travel' )
    AND t.assignment_group NOT IN ( 'FMS Travel' )
    --AND m.sr_created_on_date >= ( current_date - 3000 )
    AND extract(year from t.sys_updated_on ) >= (extract( year from current_date )-5)