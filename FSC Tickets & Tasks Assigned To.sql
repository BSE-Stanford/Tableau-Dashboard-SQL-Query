...
WHERE
    ...
    AND t.assignment_group in ( 'FMS Financial Support Center-LD (Secure)', 'FMS Evolve Financial Reporting (EFR) Support (Secure)', 'FMS Payroll (Secure)',
    'FMS Financial Support Center (FSC)', 'FMS Financial Support Center-FFIT (Secure)', 'FMS Financial Support Center (FSC) (Secure)' )
--and parent is not null
   AND extract(year from r.sys_created_on ) >= (extract( year from current_date )-2)
UNION ALL
SELECT
    ROW_NUMBER()
    OVER(PARTITION BY r.number, r.definition
...
WHERE
    ...
    AND ( m.assignment_group in ( 'FMS Financial Support Center-LD (Secure)', 'FMS Evolve Financial Reporting (EFR) Support (Secure)', 'FMS Payroll (Secure)',
    'FMS Financial Support Center (FSC)', 'FMS Financial Support Center-FFIT (Secure)', 'FMS Financial Support Center (FSC) (Secure)' )
--and parent is not null
          OR EXISTS (
        SELECT
...
--and parent is not null
    ...
    AND extract(year from r.sys_updated_on ) >= (extract( year from current_date )-2)